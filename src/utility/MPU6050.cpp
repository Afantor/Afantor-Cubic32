 ...
 * \class   MPU6050
 * \brief   Driver for MPU6050 module.
 * @file    MPU6050.cpp
 * @author  Afantor
 * @version V1.0.0
 * @date    2018/05/03
 ...
 *
 * \par Description
 * This file is a drive for MPU6050 module, It supports MPU6050 V1.0 device provided
 * by Afantor.
 *
 * \par Method List:
 *
 *    1. void MPU6050::setpin(uint8_t AD0, uint8_t INT)
 *    2. void MPU6050::begin(void)
 *    3. void MPU6050::update(void)
 *    4. void MPU6050::fast_update(void)
 *    5. uint8_t MPU6050::getDevAddr(void)
 *    6. double MPU6050::getAngleX(void)
 *    7. double MPU6050::getAngleY(void)
 *    8. double MPU6050::getAngleZ(void)
 *    9. double MPU6050::getGyroX(void)
 *    10. double MPU6050::getGyroY(void)
 *    11. double MPU6050::getGyroZ(void)
 *    12. double MPU6050::getTemperature(void)
 * \par History:
 * <pre>
 * `<Author>`         `<Time>`        `<Version>`        `<Descr>`
 *  Blue God         2018/05/02         1.0.0         rebuild the lib.
 * </pre>
 *
 * @example MPU6050Test.ino
 */
 ...
 */
MPU6050::MPU6050(uint8_t AD0, uint8_t INT)
{
 ...
 */
MPU6050::MPU6050(uint8_t AD0, uint8_t INT, uint8_t address)
{
 ...
 *   None.
 * \par Others
 *   Set global variable _AD0, _INT
 */
void MPU6050::setpin(uint8_t AD0, uint8_t INT)
{
  _AD0 = AD0;
  _INT = INT;
}
 ...
 *   None
 */
uint8_t MPU6050::getDevAddr(void)
{
 ...
 *   Z-axis angle value is integral of Z-axis angular velocity.
 */
double MPU6050::getAngle(uint8_t index)
{
  if(index == 1)
  {
    return getAngleX();
 ...
 *   deviceCalibration
 * \par Description
 *   Calibration function for the MPU6050. 
 * \param[in]
 *   None
 * \par Output
 *   None
 ...
 */
int8_t MPU6050::writeReg(int16_t reg, uint8_t data)
{
 ...
 */
int8_t MPU6050::readData(uint8_t start, uint8_t *buffer, uint8_t size)
{
  int16_t i = 0;
  int8_t return_value = 0;
 ...
 */
int8_t MPU6050::writeData(uint8_t start, const uint8_t *pData, uint8_t size)
{
  int8_t return_value = 0;
  Wire.beginTransmission(Device_Address);
  return_value = Wire.write(start); 
